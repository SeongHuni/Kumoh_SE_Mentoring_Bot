...
**유사도 threshold로 거르지 않습니다** — top-5는 점수가 아무리 낮아도 그대로 답변 생성 프롬프트로 들어갑니다. `config.py`에 `RAG_MIN_SCORE`(기본 0.0)가 남아있지만 `rag.py` 어디에서도 쓰이지 않는, 옛 구조의 흔적입니다. 원래는 점수 컷오프로 "근거 부족"을 걸렀지만, 근거가 실제로 질문에 맞는지 판단하는 역할을 `answer_rules.py`의 프롬프트 규칙(참고 자료에 실제로 있는 내용만 근거로 쓴다 · 단어만 겹치고 적용 대상이 다르면 근거로 안 씀)으로 옮기면서 뺐습니다. 순위 조정(어떤 5개를 뽑을지)은 가중치·다양성이, 근거 충분성 판단(그 5개로 답할 수 있는지)은 LLM이 나눠서 맡는 구조입니다.
