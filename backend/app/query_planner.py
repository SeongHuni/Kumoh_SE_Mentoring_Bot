...
- 약어(MT, TA, SW, AI 등)나 짧은 명사만 있으면 소속·주제 단어를 덧붙인다.
  붙이는 것은 "그 말이 다른 뜻과 겹쳐서 혼자서는 무엇인지 알 수 없는" 경우뿐이다.
    "MT 언제야?"        -> "소프트웨어전공 학과 MT 일정"   (TA 멘토 프로그램과 겹침)
    "과잠 신청"          -> "소프트웨어전공 학과 과잠바 신청"
    "SEcon 언제야"       -> "소프트웨어전공 SEcon 행사 일정"

  질문 원문에 "학과" 라는 말이 있다고 해서 그걸 "소프트웨어전공"으로
  바꿔 쓰지 않는다. 주제어가 이미 구체적이면 "학과"는 그냥 지운다.
    "학과 건물 와이파이 안 터지면 신고는 어디로 해?" -> "와이파이 문제 신고"
    "졸업 전까지 학과 진로교육 몇 과목 들어야 해?"   -> "졸업 진로교육 과목 수"
  실측: 위 두 질문에서 "소프트웨어전공"을 붙였더니 학과 소개 문서가 앞을
  가리고 정답이 각각 54위, 22위로 밀렸다. "와이파이", "진로교육"은
  다른 뜻과 겹치지 않는 구체적인 말이라 애초에 붙일 필요가 없었다.
...
- 학과 이름은 검색어에서 뺀다.
  ...
  같은 학과를 가리키는 말은 모두 아래와 같다. 어느 것이 나오든 지운다.
    ...
    소프트웨어학과 / 컴소 / 컴소공 / 컴퓨터공학부(전공 구분 없이 쓰인 경우) / 학과
  "SE" 는 여기 넣지 않는다. 코퍼스에서 SE 는 거의 전부 SEcon(행사),
  SE Board, 'SERVER'(학생회 이름) 의 일부이지 학과 약칭으로 쓰이지 않는다.
  "SE" 를 학과 이름으로 취급하면 관계없는 문서가 섞여 들어온다.
