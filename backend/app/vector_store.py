...
class ChromaVectorStore:
    ...
    url 이 주어지면 HTTP 서버에, 아니면 로컬 경로에 붙는다.
    둘을 동시에 쓸 수는 없다. 서버가 SQLite 파일을 잠그기 때문에,
    같은 chroma-data 를 PersistentClient 로도 열면 잠금 오류가 난다.
    서버를 띄워 쓰는 것이 기본이다.
    """

    def __init__(
        self, path: Path, collection_name: str, url: str | None = None
    ) -> None:
