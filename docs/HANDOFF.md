...
- 학과 사이트는 `sub0101`, `sub0102`, `sub0105_2`, `sub0104`, `sub0401`, `sub0504`만 수집한다. `sub0101`은 전공소개 섹션만, `sub0104`는 역사 참고 정보만 보존한다. 학과 게시판과 나머지 학과 URL을 후보·canonical·인덱스에 복원하거나 추가하지 않는다.
...
- 6건 후보는 허용 범위가 멘토링용 정적 페이지로 한정되어 10개의 `empty_topic` 경고가 난다. 검토·승격 시 이를 범위 한계로 명시한다.
- 삭제 전 평가 31/31은 역사 기록이며 현재 빈 local snapshot의 회귀 통과가 아니다. 실시간 공식 사이트나 OpenAI 품질을 보증하지 않는다.
