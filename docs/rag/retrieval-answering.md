...
- `document_type=historical`은 과거 참고 근거로만 사용하며, 최근 공지·최신성 경쟁과 `recent_notices`에서 제외한다. 청크 header의 “현재 수치·현황 아님”을 답변 맥락에 보존한다.
...
- local provider는 `확인한 최신 공지`, 정리된 제목, 분류·게시일, 불릿형 `핵심 내용`, 자료 번호와 `원문 확인` 순서로 답한다. 원문 제목의 선행 대괄호 태그는 분류 metadata로 분리하고 장식용 별표는 표시 답변에서 제거한다.
