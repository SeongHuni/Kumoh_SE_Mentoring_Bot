...
async function handleQuestion(ctx, question) {
  const { apiKey, collection, session, options, scoring } = ctx;
  const t0 = Date.now();
  ...
  // 이벤트 가중치 + 시간 감쇠. 임베딩이 모르는 '학과에서 중요한 정보'를 사람이 지정하고,
  // 같은 이벤트의 과거 공지가 최신을 밀어내지 않도록 오래된 문서를 소폭 감점한다.
  // 다양성보다 먼저 적용해야 보정된 순위 기준으로 자리가 배분된다.
...
async function main() {
  ...
  console.log(`${C.bold}소프트웨어전공 안내 챗봇${C.reset}`);
  ...
  console.log(`${C.dim}중요도 규칙 ${scoring.rules.length}개 · 시간 감쇠 연 -${scoring.recency.decay_per_year}${C.reset}`);
  console.log(`${C.dim}이어지는 질문을 그대로 물어보세요. /help 로 명령어 확인${C.reset}\n`);

  const session = new Session();
  const ctx = { apiKey, collection, session, options, scoring };
